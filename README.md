...
There is an issue with the lambda, dynamo streams doesn't ensure that your keys will be in the 'right' order. Most of the time it dynamodb stream give object with primary key first and then secondary. BUT it doesn't do it 100% of the time. That's why we came with the idea that a mapping to fix that was needed.

What does the mapping script do ?

It go through all your dynamodb, looking for enabled dynamo streams, list them, get mapping of each table and store it into a json. This json will then be used to check if the parameters are send in the right order. If not, use the right key according to the mapping.
...
This script get all dynamostream linked to the function DynamoToES. If your function
